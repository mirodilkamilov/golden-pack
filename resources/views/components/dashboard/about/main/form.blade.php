...
<div class="row">
         ...
         <div class="tab-pane @if($loop->first) active @endif tab-pane-{{$lang}}"
            ...
            <div class="col-lg-12 col-md-12">
               <fieldset class="form-label-group">
                  <textarea name="about_description[{{ $lang }}]"
                            class="form-control @error("about_description.$lang") is-invalid @enderror"
                            id="description-{{ $lang }}"
                            placeholder="{{ __('Description') }} ({{ $lang }})"
                            spellcheck="false">{{ old("about_description.$lang") ?? $content?->about_description[$lang] ?? '' }}</textarea>
   ...
   <div class="col-lg-6 col-md-6">
      ...
      <fieldset class="form-group"
         ...
         @if(isset($content?->about_image))
            <img src="{{ $content->about_image }}" class="placeholder" alt="placeholder" style="width: 300px;">
         @endif
      </fieldset>
   </div>
</div>
